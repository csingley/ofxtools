...
# local imports
from ofxtools.Client import OFXClient, StmtRq, CcStmtRq, InvStmtRq, StmtEndRq, CcStmtEndRq
...
class OFXClientV1TestCase(unittest.TestCase):
    @property
    def client(self):
        return OFXClient("https://example.com/ofx", org="FIORG", fid="FID",
                         version=103, bankid='123456789',
                         brokerid='example.com')

    @property
    def stmtRq0(self):
        return StmtRq(acctid='111111', accttype='CHECKING',
                      dtstart=datetime(2017, 1, 1, tzinfo=UTC),
                      dtend=datetime(2017, 3, 31, tzinfo=UTC))

    @property
    def stmtRq1(self):
        return StmtRq(acctid='222222', accttype='SAVINGS',
                      dtstart=datetime(2017, 1, 1, tzinfo=UTC),
                      dtend=datetime(2017, 3, 31, tzinfo=UTC))

    @property
    def ccStmtRq(self):
        return CcStmtRq(acctid='333333',
                        dtstart=datetime(2017, 1, 1, tzinfo=UTC),
                        dtend=datetime(2017, 3, 31, tzinfo=UTC))

    @property
    def invStmtRq(self):
        return InvStmtRq(acctid='444444',
                         dtstart=datetime(2017, 1, 1, tzinfo=UTC),
                         dtend=datetime(2017, 3, 31, tzinfo=UTC))

    @property
    def stmtEndRq(self):
        return StmtEndRq(acctid='111111', accttype='CHECKING',
                         dtstart=datetime(2017, 1, 1, tzinfo=UTC),
                         dtend=datetime(2017, 3, 31, tzinfo=UTC))

    @property
    def ccStmtEndRq(self):
        return CcStmtEndRq(acctid='222222',
                           dtstart=datetime(2017, 1, 1, tzinfo=UTC),
                           dtend=datetime(2017, 3, 31, tzinfo=UTC))

    def _testRequest(self, fn, *args, **kwargs):
        with patch.multiple('urllib.request', Request=DEFAULT, urlopen=DEFAULT) as mock_urllib:
    ...
    def testRequestStatements(self):
        data = self._testRequest(self.client.request_statements, 'elmerfudd', 't0ps3kr1t', self.stmtRq0)
    ...
    def testRequestStatementsEmpty(self):
        data = self._testRequest(self.client.request_statements, 'elmerfudd', 't0ps3kr1t')
    ...
    def testRequestStatementsUnclosedTags(self):
        data = self._testRequest(self.client.request_statements, 'elmerfudd', 't0ps3kr1t', self.stmtRq0, close_elements=False)
    ...
    def testRequestStatementsMultipleMixed(self):
        data = self._testRequest(self.client.request_statements, 'elmerfudd', 't0ps3kr1t', self.stmtRq0, self.stmtRq1, self.ccStmtRq, self.invStmtRq)
    ...
    def testRequestEndStatements(self):
        data = self._testRequest(self.client.request_end_statements, 'elmerfudd', 't0ps3kr1t', self.stmtEndRq)
    ...
    def testRequestEndStatementsMultipleMixed(self):
        data = self._testRequest(self.client.request_end_statements, 'elmerfudd', 't0ps3kr1t', self.stmtEndRq, self.ccStmtEndRq)
    ...
    def testRequestEndStatementsEmpty(self):
        data = self._testRequest(self.client.request_end_statements, 'elmerfudd', 't0ps3kr1t')
    ...
    def testRequestAccounts(self):
        data = self._testRequest(self.client.request_accounts, 'elmerfudd', 't0ps3kr1t', datetime(1954, 10, 31, tzinfo=UTC))
