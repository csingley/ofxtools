...
# 3rd party imports
from sqlalchemy import Column, Enum, Integer, String, Text, ForeignKey, event, and_
...
# local imports
from ofxtools.ofxalchemy.Types import OFXNumeric, OFXDateTime, OFXBoolean, NagString
        ...
        def id(cls):
            if has_inherited_table(cls):
                return Column(
                    Integer,
                    ForeignKey(
                        "%s.id" % parent_table, onupdate="CASCADE", ondelete="CASCADE"
                    ),
                    primary_key=True,
                )
            else:
...
class Balance(object):
    ...
    def acctfrom_id(cls):
        return Column(
            Integer,
            ForeignKey("acctfrom.id", onupdate="CASCADE", ondelete="CASCADE"),
            primary_key=True,
        )
...
class INVBAL(Base):
    ...
    acctfrom = relationship(
        "INVACCTFROM",
        backref=backref("invbals", cascade="all, delete-orphan", passive_deletes=True),
    )
...
class BAL(Balance, CURRENCY, Base):
    ...
    baltype = Column(
        Enum("DOLLAR", "PERCENT", "NUMBER", name="baltype"), nullable=False
    )
...
class SECID(object):
    ...
    def secinfo_id(cls):
        return Column(
            Integer,
            ForeignKey("secinfo.id", onupdate="CASCADE", ondelete="CASCADE"),
            nullable=False,
        )
...
class DEBTINFO(SECINFO):
    ...
    couponfreq = Column(
        Enum("MONTHLY", "QUARTERLY", "SEMIANNUAL", "ANNUAL", "OTHER", name="couponfreq")
    )
    callprice = Column(OFXNumeric())
    yieldtocall = Column(OFXNumeric())
    dtcall = Column(OFXDateTime)
    calltype = Column(Enum("CALL", "PUT", "PREFUND", "MATURITY", name="calltype"))
...
class PORTION(Base):
    ...
    mfinfo_id = Column(
        Integer,
        ForeignKey("mfinfo.id", onupdate="CASCADE", ondelete="CASCADE"),
        primary_key=True,
    )
    mfinfo = relationship(
        "MFINFO",
        backref=backref(
            "mfassetclasses", cascade="all, delete-orphan", passive_deletes=True
        ),
    )

    # Elements from OFX spec
    assetclass = Column(Enum(*ASSETCLASSES, name="assetclass"), primary_key=True)
...
class FIPORTION(Base):
    ...
    mfinfo = relationship(
        "MFINFO",
        backref=backref(
            "fimfassetclasses", cascade="all, delete-orphan", passive_deletes=True
        ),
    )

    # Elements from OFX spec
    fiassetclass = Column(Enum(*ASSETCLASSES, name="assetclass"), primary_key=True)
    percent = Column(OFXNumeric(), nullable=False)
...
class BANKTRAN(ORIGCURRENCY):
    ...
    def acctto_id(cls):
        return Column(
            Integer, ForeignKey("acctto.id", onupdate="CASCADE", ondelete="CASCADE")
        )

    @declared_attr
    def payee_name(cls):
        return Column(
            String(32), ForeignKey("payee.name", onupdate="CASCADE", ondelete="CASCADE")
        )
...
class STMTTRN(BANKTRAN, Base):
    ...
    acctfrom = relationship(
        "ACCTFROM",
        backref=backref("stmttrns", cascade="all, delete-orphan", passive_deletes=True),
    )
    ...
    def acctto(cls):
        return relationship(
            "ACCTTO",
            backref=backref(
                "stmttrns", cascade="all, delete-orphan", passive_deletes=True
            ),
        )
...
class INVBANKTRAN(BANKTRAN, Base):
    ...
    acctfrom = relationship(
        "INVACCTFROM",
        backref=backref(
            "invbanktrans", cascade="all, delete-orphan", passive_deletes=True
        ),
    )
...
class INVTRAN(Inheritor("invtran"), Base):
    ...
    def acctfrom(cls):
        return relationship(
            "INVACCTFROM",
            backref=backref(
                "invtrans", cascade="all, delete-orphan", passive_deletes=True
            ),
        )
...
class INVBUYSELL(SECID, ORIGCURRENCY):
    ...
    unitprice = Column(OFXNumeric(), CheckConstraint("unitprice >= 0"), nullable=False)
...
class INVSELL(INVBUYSELL):
    ...
    loanid = Column(String(length=32))
    statewithholding = Column(OFXNumeric(), CheckConstraint("statewithholding >= 0"))
...
class BUYOPT(INVBUY, INVTRAN):
    optbuytype = Column(
        Enum("BUYTOOPEN", "BUYTOCLOSE", name="obtbuytype"), nullable=False
    )
    shperctrct = Column(Integer, nullable=False)
...
class CLOSUREOPT(SECID, INVTRAN):
    optaction = Column(
        Enum("EXERCISE", "ASSIGN", "EXPIRE", name="optaction"), nullable=False
    )
    units = Column(OFXNumeric(), nullable=False)
    shperctrct = Column(Integer, nullable=False)
    subacctsec = Column(Enum(*INVSUBACCTS, name="subacctsec"), nullable=False)
...
class SELLDEBT(INVSELL, INVTRAN):
    sellreason = Column(
        Enum("CALL", "SELL", "MATURITY", name="sellreason"), nullable=False
    )
    accrdint = Column(OFXNumeric())
...
class SELLOPT(INVSELL, INVTRAN):
    optselltype = Column(
        Enum("SELLTOCLOSE", "SELLTOOPEN", name="optselltype"), nullable=False
    )
    shperctrct = Column(Integer, nullable=False)
    relfitid = Column(String(length=255))
...
class INVPOS(Inheritor("invpos"), SECID, CURRENCY, Base):
    ...
    acctfrom = relationship(
        "INVACCTFROM",
        backref=backref("invposs", cascade="all, delete-orphan", passive_deletes=True),
    )
    dtasof = Column(OFXDateTime)
